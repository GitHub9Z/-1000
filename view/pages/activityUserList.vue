...
<!-- 						<view class="padding" style="padding-bottom: 0;" v-if="activity.tag">
						...
						<view class="text-gray text-sm padding" style="display: flex; align-items: center; line-height: 20px;">
							<view style="flex: 1; display: flex;">
								...
								<text style="color: #333333">{{activity.user[0].user_name}}</text>
								<text style="margin-left: 10px;">发布于{{getDateDiff(activity.create_time)}}</text>
							</view>
	...
	export default {
		...
		methods: {
			...
			handleItemClick(activity) {
				uni.showModal({
					title: '提示',
					content: `是否申请加入${activity.user_name}？${this.get_user_info.invite_id ? '(申请后将退出原俱乐部且会员信息不再保留，且在此之前的俱乐部申请将自动取消)' : ''}`,
					...
					success: (res) => {
						if (res.confirm) {
							uni.request({
								url: 'https://www.imgker.com/venus/user/bind_activity', //仅为示例，并非真实接口地址。
								data: {
